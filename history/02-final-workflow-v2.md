...
4. **Context budget is real.** Keep session work under a token budget, not
   a raw percentage. System prompt + tools + MCP burn 30-40k tokens before
   you type. Start closing around 100k and compact around 120k, not near the
   model limit.
...
6. **Watch the context meter.** Around 100k tokens the harness should warn
   you (hook, see §5). Don't push near the model limit.
