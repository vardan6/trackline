...
4. `docs/current-state.md` and `docs/roadmap.md`.
...
Invalid or not worth doing:
  - <finding>: <one-line reason>
Recommended immediate action: <e.g. fix must_fix_now then /cycle-close>
...
- If no must-fix items → suggest `/cycle-close` or `/session-close` and stop.
