...
4. **Context budget is real.** Track both percent and k-tokens. On a 200k
...
3. **Implement** — append meaningful state changes to `activeContext.md` as
   ...
   play-by-play.
4. **Watch the context meter** — at ~50% the harness warns (see §6), including
   k-tokens: 100k on 200k, 200k on 400k, 500k on 1M, or 525k on GPT-5.5's
   1,050k. Don't push past ~60% before handing off.
...
- Pick a concrete `/handoff` trigger threshold (50% recommended: 100k on
  200k, 200k on 400k, 500k on 1M, 525k on GPT-5.5's 1,050k).
