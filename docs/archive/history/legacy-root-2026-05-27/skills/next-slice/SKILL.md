...
- After finishing a step → `/cycle-close`.
- Plan is unclear → `/planning-capture` or `/grill-me` first.

## Inputs (read order, stop when confident)

1. `docs/current-state.md` and `docs/roadmap.md` (already in context from `/session-open` if used).
2. Recent git state.
...
1. Identify candidate next steps from `roadmap.md` and `current-state.md`.
...
- After implementing the slice, do not chain into another slice automatically → `/cycle-close`.
